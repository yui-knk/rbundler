...
test_bundle(
  desc = "Bundling a package with git sources creates a bundle directory with the project installed and all dependencies installed.",
  package = mock_packages[["gitdependencies"]],
  expected_dependencies=list(dependency)
)

test_bundle(
  desc = "Bundling a package with url sources creates a bundle directory with the project installed and all dependencies installed.",
  package = mock_packages[["urldependencies"]],
  expected_dependencies=list(dependency)
)
            
dir.create(file.path(test_path, 'lib'))
writeBin(
  RCurl::getBinaryURL('http://cran.r-project.org/src/contrib/tempdisagg_0.23.tar.gz'),
  file.path(test_path, 'lib/tempdisagg_0.23.tar.gz')
)

test_bundle(
  desc = "Bundling a package with file sources creates a bundle directory with the project installed and all dependencies installed.",
  package = mock_packages[["filedependencies"]],
  expected_dependencies=list(dependency)
)
